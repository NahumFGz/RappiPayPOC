...
import { loginRefreshApi } from './auth/authAPI'
...
function createAxiosBlobAuthInstance() {
  const getToken = () => useAuthStore.getState().token
  const setToken = (token) => useAuthStore.getState().setToken(token)
  const cleanStore = () => useAuthStore.getState().cleanStore()
...
export { createAxiosInstance, createAxiosAuthInstance, createAxiosBlobAuthInstance }
